...
files = dir('*MPT2001*.mat');                   % where your .mat files are
...
% files = files(~~contains({files.name}, '200127'));
...
%inputs:
files = dir('*FTD*aSpikes_-11*.mat*');  % where your .mat files are
...
method = 'tileCoef';
sync_win_s = 0.175; %synchroncity window in seconds; e.g. 1 is +/- 1 s is considered synchronous (2DeltaT = 2 s)
rec_length_s = 360;
fs = 25000;
