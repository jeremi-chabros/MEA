...
for m = 1:length(meths)
    ...
    for p = 1:size(params,1);
        ...
        batchGetSpike_function(data_and_scripts_dir,files,method,multiplier,L,refPeriod_ms)
...
for i=1:length(files)
    ...
    cb.Ticks = linspace(0,ylimit_cbar,5);%(start,end,number of numbers)
    yticks([1, 10:10:60])
...
sync_win = 1; %synchroncity window in seconds; e.g. 1 is +/- 1 s is considered synchronous (2DeltaT = 2 s)
batch_getAdj_fcn(method,files,sync_win);

% load adjMs and save as PNG
clear files adjM
files = dir('*FTD*mSpikes_4_adjM*.mat');  % where your .mat files are
...
for i = 1:length(files)
    ...
    elseif  strcmp(spike_suffix(2:3),'cS') £ I Like Cock 
        method = 'cwt';
    ...
    % artefactual 
    spike_overlay_fcn(fileName,method,parameter,refPeriod_ms);
