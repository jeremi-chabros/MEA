...
files = dir('*200127*.mat');                   % where your .mat files are 
...
%files = files(~contains({files.name}, 'Slice5'));
%files = files(~contains({files.name}, 'Slice6'));
%files = files(~contains({files.name}, 'Slice7'));
%files = files(~contains({files.name}, 'Slice8'));
...
%inputs:
files = dir('*200127*mSpikes_4*.mat*');  % where your .mat files are
...
cb.Box = 'off';
set(gca, 'FontSize', 14)
caxis([0,4]) %set colorbar axis limits; also adjusts colour 
%below command does not adjust colour hence need for caxis command
%remove decimal ticks e.g. 0.5 Hz
cb.Ticks = linspace(0,4,5);%(start,end,number of numbers)
yticks([1, 10:10:60])
...
clear files adjM
files = dir('*200127*mSpikes_4*_adjM*.mat');  % where your .mat files are
%files = files(~contains({files.name}, 'Slice17'));
%files = files(~contains({files.name}, 'Slice16'));
%files = files(~contains({files.name}, 'Slice15'));
%files = files(~contains({files.name}, 'Slice14'));
...
%% save heatMaps
clear all
files = dir('*200127*mSpikes_5.mat*');  % where your .mat files are
%files = files(~contains({files.name}, 'Slice17'));
%files = files(~contains({files.name}, 'Slice16'));
%files = files(~contains({files.name}, 'Slice15'));
%files = files(~contains({files.name}, 'Slice14'));
...
close all; clear all
files = dir('*200127*Slice*mSpikes_5.mat*');  % where your .mat files are
%files = files(~contains({files.name}, 'Slice17'));
%files = files(~contains({files.name}, 'Slice16'));
%files = files(~contains({files.name}, 'Slice15'));
%files = files(~contains({files.name}, 'Slice14'));
...
spike_overlay_fcn(fileName,method,parameter);
