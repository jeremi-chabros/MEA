...
function plotSpikes_fcn(fileName,method,parameter,refPeriod_ms)
...
if ~~exist(strcat(fileName(1:end-4),'_channelXY_',int2str(channels(electrode_to_plot)),...
        '_spike_overlay.png'))
    
    fprintf(strcat('\n \n',fileName(1:end-4),'\n',...
        '_channelXY_',int2str(channels(electrode_to_plot)),...
        '_spike_overlay.png','\n \n already done! \n \n'))
    ...
    if  ~~exist(strcat(raw_dat_fileName(1:end-4),'_Filtd.mat'))
        ...
        finalData =     filteredMatrix(:,electrode_to_plot);
        clear filteredMatrix
        spikeTrain =    spikeMatrix(:,electrode_to_plot);
        threshold = threshold;
    else
        load(raw_dat_fileName);
        [spikeTrain, finalData, threshold] = detectSpikes(dat(:,electrode_to_plot),method, multiplier,L,refPeriod_ms);
    end
...
if ~~exist(strcat(fileName(1:end-4),'_channelXY_',int2str(channels(electrode_to_plot)),...
    ...
    if  ~exist('spikeTrain')
        ...
        if  ~~exist(strcat(raw_dat_fileName(1:end-4),'_Filtd.mat'))
            ...
            load(raw_dat_fileName);
            [spikeTrain, finalData, threshold] = detectSpikes(dat(:,electrode_to_plot),method, multiplier,L);
        end
        disp('acquired filtered data')
    end
    ...
    if  contains(fileName,'_') %remove underscores for title
        ...
        title({[int2str(length(samples_index)/fs),'{ s of recording from:}'],[fileName1],...
            ['{channel (MCS xy coordinate): }',num2str(channels(electrode_to_plot))],['{ (scalebars: horizontal }',num2str(sb.XLen),'{ s}','{ vertical }',...
            int2str(sb.YLen),'{ uV}',')'],[' ']});
    else
        title({[int2str(length(samples_index)/fs),'{ s of recording from:}'],[fileName1],...
            ['{channel (MCS xy coordinate): }',num2str(channels(electrode_to_plot))],['{ (scalebars: horizontal }',num2str(sb.XLen),'{ s}','{ vertical }',...
...
if ~~exist(strcat(fileName(1:end-4),'_channelXY_',int2str(channels(electrode_to_plot)),...
    ...
    if  contains(fileName,'_') %remove underscores for title
        ...
        title({[int2str(length(samples_index)/fs*1000),'{ ms of recording from:}'],[fileName1],...
            ['{channel (MCS xy coordinate): }',num2str(channels(electrode_to_plot))],['{ (scalebars: horizontal }',num2str(floor(sb.XLen)),'{ ms}','{ vertical }',...
            int2str(sb.YLen),'{ uV}',')'],[' ']});
    else
        title({[int2str(length(samples_index)/fs*1000),'{ ms of recording from:}'],[fileName1],...
            ['{channel (MCS xy coordinate): }',num2str(channels(electrode_to_plot))],['{ (scalebars: horizontal }',num2str(floor(sb.XLen)),'{ ms}','{ vertical }',...
...
if ~~exist(strcat(fileName(1:end-4),'_channelXY_',int2str(channels(electrode_to_plot)),...
    ...
    if  contains(fileName,'_') %remove underscores for title
        ...
        fileName1=strcat('{',fileName1(1:end-4),'}');
        title({[int2str(length(samples_index)/fs*1000),'{ ms of recording from:}'],[fileName1],...
            ['{channel (MCS xy coordinate): }',num2str(channels(electrode_to_plot))],['{ (scalebars: horizontal }',num2str(sb.XLen),'{ ms}','{ vertical }',...
            int2str(sb.YLen),'{ uV}',')'],[' ']});
    else
        title({[int2str(length(samples_index)/fs*1000),'{ ms of recording from:}'],[fileName1],...
            ['{channel (MCS xy coordinate): }',num2str(channels(electrode_to_plot))],['{ (scalebars: horizontal }',num2str(sb.XLen),'{ ms}','{ vertical }',...
