...
files = dir('*MPT200109_2C*cSpikes_L0.mat');  % where your .mat files are
    ...
    if spikes_only ~= 1
        ...
        if length(active_chanIndex)<3 %set all output to 0 etc. if no activity
            ...
            if ~exist(strcat(files(file).name(1:end-4), '_adjM_',num2str(sync_win_s),'.mat'))
                ...
                rec_length_s = length(spikeMat)/fs;
                ...
                fprintf(strcat('\n','\n','getting adjM as it was not saved or could not find save file...','\n','\n'))
                batch_getAdj_fcn(method,files(file),sync_win,num_samples,ds_rate);
 ...
 approach:
            ...
            if g_measures == 1
                ...
                for cutoff = [40 60 80]
                    ...
                    threshold = prctile(active_adjM(:), cutoff);
    ...
    disp(strcat("for file" + " ", num2str(file)," of" + " ",...
        num2str(length(files))," " + "time taken was:" + " ",...
        num2str(round(toc/60,1))," " + "minutes"))
...
method = 'cwt';
threshold = '0';
fileName = strcat(method,'_' ,threshold, '_mecp2_graph_statsLASTFOUR', '.mat');
