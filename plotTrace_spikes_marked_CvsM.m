...
electrode_to_plot=3; %not MCS ID but which column the desired E is in
...
[spikeTrain, finalData, threshold] = detectSpikes(dat(:,electrode_to_plot), 'cwt', 0,L);
pos_spikeTrain=spikeTrain(1:1500000,1);
pos_finalData=finalData(1:1500000,1);

finalData=[pre_finalData; pos_finalData];
spikeTrain=[pre_spikeTrain; pos_spikeTrain];
%% whole recording
fs=25000;
...
stim_time = [length(pre_finalData)-4000:length(pre_finalData)+4000]';
plot([stim_time stim_time], [-39 45], 'r')

%% blown up

desired_time_to_plot_in_secs=0.1;
downsample='no';
...
plot([spikePos,spikePos]', [29 39], 'k') %add ' to flip x axis matrix if only 2 spikes
