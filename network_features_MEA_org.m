...
burst detection and functional connectivity:    https://github.com/Timothysit/mecp2
cf Giandomenico et al., 2019. Nat. Neurosci. 22, 669–679.

graph metrics:                                  https://sites.google.com/site/bctnet/measures/list
cf Rubinov, M., Sporns, O., 2010. NeuroImage, Computational Models of the Brain 52, 1059–1069.

 
input = .mat data files containing the following variables:
    #n samples x n channels spike matrix (the name of this variable must
    have the suffix 'Spikes'. This can be a sparse matrix to save memory

    #a row vector called 'channels' of length equal to the n of channels in
    the spike matrix (i.e. a 1 x n channels vector). This vector gives the
    ID number of the electrode in column-row format (e.g. electrode in 
    column 3, row 2 of the grid on the MEA would have an ID of 32). The
    the position of the electrode within the vector must correspond to the
    spike matrix. That is, if electrode ID 32 is 7th column in the channel 
    vector, its spike train must be 7th column in the spike matrix

    #see the list of options that can be edited below

output:
    #a structural array where each row is a recording; the first column is 
    the recording name, the second column is the spike count in each 
    electrode (this is important to save as it can be used to check for 
    outliers such as high spike counts at DIV07 that should be excluded). 
    After that, each column is a network feature. 

this outut will be saved as a mat file
to edit the filename, edit the final section of this script
under the 'save' section
%}

clear all
try
close(' 0%')%closes progress bar
catch
end

%turn off burst detection warning: 'MATLAB:nearlySingularMatrix'
%found name of warning using:
%w=warning('query','last');
%id = w.identifier;
%warning('off',id);
warning('off','MATLAB:nearlySingularMatrix');

% note files must have the following format EEEYYMMDD_LI_DIV##
%where EEE is the experiment code e.g. MPT
% YYMMDD is the date, year first
% LI is the littermate number and the MEA label e.g. 4B
% DIV## is the age of the culture e.g. DIV28
%e.g. MPT190114_4B_DIV28
...
%{
the defaults  used are given in each section; other methods are available
other available methods are suggested in the comments in the relevant
section. Most of these can be changed by editing the option called by this 
script. Some require editing of the function itself (i.e. another script 
called by this script). I am currently working on editing the functions
called here to add custom input options so that the user does not have to
leave this script in order to change analysis options

burst detection method = Bakkum 2014
minimum n spikes in a burst = 30
min n channels required for network burst = 3

functional connectivity method = spike time tiling coefficient 
synchronicity window = 50ms
edge threshold = 0.5 (absolute)
...
for file = 1:length(files)
    ...
    channels=load(strcat(files(file).name(1:18),'.mat'),'channels'); 
...
%skip file if no spikes
if sum(sum(spikeMat))==0
    fprintf('no spikes in this file')
    output(file).mean_FR       = 0 ;
    output(file).median_FR     = 0 ;
    output(file).N_active_Es   = 0 ;
    output(file).CVIBI         = NaN ;
    output(file).BurstRate     = 0 ;
    output(file).frac_in_burst = 0 ;
...
try %adding in try loop here as sometimes burst detection doesnt work if there arent enough spikes
    %not sure why 
...
%removed below loop as CVIBI=0 would suggest low CVIBI - better to keep as
%NaN and exclude
%if isnan(output(file).CVIBI) %if there are no bursts, CVIBI will be 0/0=NaN
%    output(file).CVIBI=0; %change to 0
%else
%end
    % clear unneeded vars
...
threshold = prctile(adjM(:), 60); % get the top 40% percentile 
%threshold = 0.5;
...
%R = makerandCIJ_und(Nnodes,Nedges); % use this if the degree distribution
%cannot be preserved - this may occur if too few nodes or edges
...
toc
fprintf('\n \n <strong> FILE DONE </strong> \n \n \n')
progressbar(file/length(files));

end % end of loop that skips those with no spikes
...
clear channels   edges    tarly 
        ...
        threshold = ''; %spike detection parameter used
        fileName = strcat(method, threshold, 'output_24-10-2019_organoid', '.mat'); 
